    ...
    for i in range(kernel_size):
        for j in range(kernel_size):
            ...
            x_term = (x ** 2 - sigma_x ** 2) / \
                (2 * math.pi * sigma_x ** 5 * sigma_y)
            y_term = (y ** 2 - sigma_y ** 2) / \
                (2 * math.pi * sigma_y ** 5 * sigma_x)
...
def word_segmentation(img, kernel_size=25, sigma=11, theta=7):
    ...
    words_contours, _ = cv2.findContours(
        np.invert(binary_image), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
